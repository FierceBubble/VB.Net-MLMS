...
Public Class SignUp
    ...
    Private Sub CreateID()
        ...
        Try
            conn.Open()
            Dim cmd As MySqlCommand = New MySqlCommand("insert into mlms.customer 
                                     (customerid, firstname, lastname, gender, dob, password) values 
                                     ('" & IDBox.Text & "','" & FNameBox.Text & "','" & LNameBox.Text & "',
                                     '" & Gender & "','" & DOBPicker.Text & "','" & PasswordBox.Text & "')", conn)
            Dim rdr As MySqlDataReader = cmd.ExecuteReader
    ...
    Private Sub CancelButton_Click(sender As Object, e As EventArgs) Handles CancelButton2.Click
        'Button to cancel creation of new customer ID and bring the customer back to the'
        'Sign-In page'
        Me.Close()
        SignIn.Show()
    End Sub
